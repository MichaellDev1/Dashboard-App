...
export default function CardTask ({ data, deleteTask, saveTask, handleCompleted }) {
  ...
  return data
    ? <div className='flex items-center bg-white my-3 py-5 max-w-[600px] flex-wrap rounded-2xl  justify-center px-5' style={{ boxShadow: '0px 10px 10px rgba(0 0 0 / 3%)' }}>
      ...
      <div className='ml-5 flex'>
        ...
        editable
          ? <div className='flex items-center flex-col mr-5'>
            <input type='checkbox' name='important' className='checked:bg-orange-300' id='important' ref={refCheckBox} />
          ...
          : null
            ...
            {
      editable ? <button className='ml-5 text-sm text-green-500' onClick={(() => saveTask({ id, setEditable, refTask, refCheckBox }))}>Save</button> : null
     }
            </div>
       }
