...
const getPriceCoin = ({ page = 1, limit = 20 }) => {
  return fetch(`${URL_API}${MARKET_URL}?vs_currency=usd&order=market_cap_desc&per_page=${limit}&page=${page}&sparkline=true&price_change_percentage=price_change_percentage&locale=en`)
    .then(res => res.json())
    .then(dataMarket)
}
